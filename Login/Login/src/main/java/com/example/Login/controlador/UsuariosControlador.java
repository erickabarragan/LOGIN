...
public class UsuariosControlador {
				...
				public ResponseEntity<List<Usuarios>> obtenerTodos(){
			        ...
			        @PutMapping("{id}")
			        public ResponseEntity<Usuarios> actualizar(@PathVariable("id") int id, @RequestBody Usuarios item) {
			            Optional<Usuarios> itemsOpcional = usuariosRepositorio.findById(id);
			            if (itemsOpcional.isPresent()) {
			                Usuarios itemExistente = itemsOpcional.get();
			                itemExistente.setContrasena(item.getContrasena());
			                itemExistente.setCorreo(item.getContrasena());
			                itemExistente.setEstado(item.getEstado());
			                itemExistente.setSesionActiva(item.getSesionActiva());
			                return new ResponseEntity<>(usuariosRepositorio.save(itemExistente), HttpStatus.OK);
			            } else {
